...
public class EnemyStatsEditorWindow : EditorWindow
{
    ...
    [MenuItem("Map Edition/Enemy Stats Editor")]
    ...
    [MenuItem("Map Edition/Show Positions")]
    ...
    [MenuItem("Map Edition/Show Elevation")]
    public static void ShowElevation()
    {
        ...
        for (int i = 0; i < gridGO.childCount; i++)
        {
            ...
            if (breakablepositions.Contains(position))
            {
                gridGO.GetChild(i).GetChild(1).GetComponent<SpriteRenderer>().color = Color.magenta;
                breakablepositions.Remove(position);
            }
            if (gridGO.GetChild(i).GetComponent<GridSquareScript>().isobstacle)
            {
    ...
    [MenuItem("Map Edition/Hide Characters Positions")]
